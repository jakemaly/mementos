...
async def _mock_run_research(query, on_event=None, **_kw):
    """Mock that emits events via on_event callback then returns final result."""
    ev1 = {"id": "a", "type": "brief_generated", "payload": {}, "timestamp": 0}
    if on_event:
        on_event(ev1)
    return {
...
def test_research_stream_emits_events():
    ...
    with patch("research.graph.run_research", side_effect=_mock_run_research):
